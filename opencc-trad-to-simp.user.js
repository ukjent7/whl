...
(async function () {
  ...
  const state = {
    ...
    dragging: false,        // 拖拽进行中，mouseup 时据此区分点击与拖拽
  };
  ...
  function updatePopupDirection() {
    ...
    // 判断按钮中心所在象限
    const onRight  = fabLeft + btnSize / 2 > vw / 2;
    const onBottom = fabTop  + btnSize / 2 > vh / 2;

    // 面板水平方向
    panel.style.left  = onRight  ? 'auto' : '0';
    panel.style.right = onRight  ? '0'    : 'auto';
    toast.style.left  = onRight  ? 'auto' : '0';
    toast.style.right = onRight  ? '0'    : 'auto';

    // 面板垂直方向
    const offset = btnSize + gap;
    panel.style.top    = onBottom ? 'auto' : `${offset}px`;
    panel.style.bottom = onBottom ? `${offset}px` : 'auto';
    toast.style.top    = onBottom ? 'auto' : `${offset}px`;
    toast.style.bottom = onBottom ? `${offset}px` : 'auto';
  }
  ...
  function buildUI() {
    ...
    // ── 拖拽逻辑（Pointer Events，兼容鼠标与触屏）──────────────────────────
    const DRAG_THRESHOLD = 5; // 超过此像素才视为拖拽，否则视为点击
    ...
    btn.addEventListener('pointermove', (e) => {
      ...
      // 计算新位置，限制在视口内
      const btnSize = 52, margin = 8;
      const newX = Math.min(Math.max(fabStartX + dx, margin), window.innerWidth  - btnSize - margin);
      const newY = Math.min(Math.max(fabStartY + dy, margin), window.innerHeight - btnSize - margin);
    ...
    btn.addEventListener('pointerup', async (e) => {
      ...
      if (hasDragged) {
        ...
        GM_setValue('pos_x', x);
        GM_setValue('pos_y', y);
        // 延迟重置，防止 pointerup 后的 click 事件误判
    ...
    // ── 视口 resize 时将按钮限制在可见区域内 ─────────────────────────────────
    window.addEventListener('resize', () => {
      const btnSize = 52, margin = 8;
      const curX = parseInt(fab.style.left, 10);
      const curY = parseInt(fab.style.top,  10);
      const clampedX = Math.min(Math.max(curX, margin), window.innerWidth  - btnSize - margin);
      const clampedY = Math.min(Math.max(curY, margin), window.innerHeight - btnSize - margin);
